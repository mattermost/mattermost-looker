...
view: focalboard_activity {
  ...
  dimension: md5(cast(
    ...
    coalesce(cast(activity.timestamp::date as 
    varchar
), '') || '-' || coalesce(cast(activity.user_id as 
    varchar
), '')
 ...
 as 
  ...
  measure: md5(cast(
    ...
    coalesce(cast(activity.timestamp::date as 
    varchar
), '') || '-' || coalesce(cast(activity.user as 
    varchar
), '')
 ...
 As 
    ...
    description: "The distinct count of Focalboard Activity Md5(Cast(
    
    Coalesce(Cast(Activity.Timestamp::Date As 
    Varchar
), '') || '-' || Coalesce(Cast(Activity.Users As 
    Varchar
), '')
