...
view: version_release_dates {
  ...
  dimension: supported {
    ...
    type: yesno
    sql: CASE WHEN ${release_date} >= CURRENT_DATE - INTERVAL '3 MONTHS' AND ${release_date} <= CURRENT_DATE THEN TRUE
              WHEN ${release_date} IN ('2021-01-16', '2021-07-16','2022-01-16', '2022-07-16') AND ${release_date} >= CURRENT_DATE - INTERVAL '9 MONTHS' AND ${release_date} <= CURRENT_DATE THEN TRUE
              ELSE FALSE END ;;
    #${TABLE}.supported ;;
    hidden: no
  }
